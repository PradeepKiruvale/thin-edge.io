mod actor;
use actor::HealthManagerMessageBox;
...
impl HealthMonitorBuilder {
    ...
    pub fn with_mqtt_connection(&mut self, mqtt: &mut MqttActorBuilder) -> Result<(), LinkError> {
        let subscriptions = vec!["tedge/health-check", "tedge/health-check/+"]
            .try_into()
            .unwrap();
...
impl Builder<(TedgeHealthMonitorActor, HealthManagerMessageBox)> for HealthMonitorBuilder {
    type Error = LinkError;

    fn try_build(self) -> Result<(TedgeHealthMonitorActor, HealthManagerMessageBox), Self::Error> {
        let mqtt_publisher = self.mqtt_publisher.ok_or_else(|| LinkError::MissingPeer {
            role: "mqtt".to_string(),
        })?;

        let message_box = HealthManagerMessageBox::new(self.input_receiver, mqtt_publisher.clone());

        let actor = TedgeHealthMonitorActor::new(mqtt_publisher);
