...
async fn test_convert_small_measurement() {
    let (_temp_dir, mut converter, _http_proxy) = create_c8y_converter().await;
    let measurement_topic = "tedge/measurements";
    let small_measurement_payload = create_thin_edge_measurement(20); // Measurement payload size is 20 bytes
...
#[tokio::test]
async fn test_convert_measurement_with_custom_type() {
    let (_temp_dir, mut converter, _http_proxy) = create_c8y_converter().await;
    let measurement_topic = "tedge/measurements";
    ...
    assert!(result[0]
        .payload_str()
        .unwrap()
        .contains(r#"{"type":"SmallType","temp":{"temp":{"value":120.0}}"#));
}
...
#[tokio::test]
async fn test_convert_measurement_with_type_for_child_device() {
    let measurement_topic = "tedge/measurements/child1";
    let measurement_payload = r#"{"type":"SmallType","temp": 120}"#;
    ...
    let (_temp_dir, mut converter, _http_proxy) = create_c8y_converter().await;
    let result = converter.convert(&measurement_message).await;
    ...
    assert!(payload2 .contains(
        r#"{"externalSource":{"externalId":"child1","type":"c8y_Serial"},"type":"SmallType","temp":{"temp":{"value":120.0}}"#
    ));
}
